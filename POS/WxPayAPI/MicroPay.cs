...
namespace POS.WxPayAPI
{
    ...
    {
        static ApplicationLogger logger = new ApplicationLogger(typeof(MicroPay).Name);
        /**
        * 刷卡支付完整业务流程逻辑
        * @param body 商品描述
        * @param total_fee 总金额
        ...
        * @return 刷卡支付结果
        */
        public static WxPayData Run(string device_info, string total_fee, string auth_code)
        {
            logger.Info("MicroPay:"+ "Micropay is processing...");

            string out_trade_no = WxPayApi.GenerateOutTradeNo();
            WxPayData data = new WxPayData();
            data.SetValue("auth_code", auth_code);//授权码
            data.SetValue("body", "刷卡支付");//商品描述
            data.SetValue("total_fee", int.Parse(total_fee));//总金额
            data.SetValue("out_trade_no", out_trade_no);//产生随机的商户订单号
            data.SetValue("device_info", device_info);//设备号

            WxPayData result = WxPayApi.Micropay(data, 30); //提交被扫支付，接收返回结果
            ...
            if (!result.IsSet("return_code") || result.GetValue("return_code").ToString() == "FAIL")
            {
                ...
                logger.Error("MicroPay:" + "Micropay API interface call failure, result : " + result.ToXml());
                throw new WxPayException("Micropay API interface call failure, return_msg : " + returnMsg);
            }

            //签名验证
            result.CheckSign();
            logger.Debug("MicroPay:"+ "Micropay response check sign success");

            //刷卡支付直接成功
            if (result.GetValue("return_code").ToString() == "SUCCESS" &&
                result.GetValue("result_code").ToString() == "SUCCESS")
            {
                logger.Debug("MicroPay:"+ "Micropay business success, result : " + result.ToXml());
                //return result.ToPrintStr();
                result.SetValue("trade_state", "SUCCESS");
            ...
            {
                logger.Error("MicroPay:"+ "micropay API interface call success, business failure, result : " + result.ToXml());
                // return result.ToPrintStr();
                //return "err";
                return result;
            }
           ...
           // string out_trade_no = data.GetValue("out_trade_no").ToString();
            ...
            while (queryTimes-- > 0)
            {
                ...
                {
                    logger.Debug("MicroPay:"+ "Mircopay success, return order query result : " + queryResult.ToXml());
                ...
                {
                    logger.Error("MicroPay:"+ "Micropay failure, return micropay result : " + result.ToXml());
                    //return result.ToPrintStr();
                    return result;
                }
            }
            ...
            //确认失败，则撤销订单
            logger.Error("MicroPay:"+ "Micropay failure, Reverse order is processing...");
            if (!Cancel(out_trade_no))
            {
                logger.Error("MicroPay:"+ "Reverse order failure");
