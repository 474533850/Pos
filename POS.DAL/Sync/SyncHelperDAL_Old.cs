...
{
    public class SyncHelperDAL_Old : BaseDAL
    {
        ...
        /// <returns></returns>
        private static string GetUrl(string sid, string mod, string xls, string usercode, object version)
        {
            string urlFormat = string.Format("{0}?sid={1}&mod={2}&xls={3}&usercode={4}&version={5}", baseUrl_Pull, sid, mod, xls, usercode, version ?? "");
            return urlFormat;
        }
        #endregion
        ...
        public static bool CheckConnect(out DateTime currentTime)
        {
            ...
            try
            {
                // Uri baseUrl = new Uri(ConfigurationManager.AppSettings["serviceUrl"].ToString());
                Uri url = new Uri(baseUrl, @"pos/cnet");
                string result = HttpClient.Get(url, 5000);
                currentTime = ConvertStringToDateTime(result);
                return true;
            }
            catch (Exception ex)
        ...
        public static bool SyncData(string sid, string xls, string usercode, List<string> tables)
        {
            try
            {
                Uri baseUrl = new Uri(ConfigurationManager.AppSettings["serviceUrl"].ToString());
        ...
        private static void SyncDel(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackDel, request);
        }

        private static void AsyncCallbackDel(IAsyncResult ar)
        {
            List<SyncDelDetailModel> datas = new List<SyncDelDetailModel>();
            ...
            {
                string result = sr.ReadToEnd();
                SyncResultModel<SyncDelDetailModel> data = js.Deserialize<SyncResultModel<SyncDelDetailModel>>(result);
                datas = data.datas.Where(r => AppConst.AllTableNames.Contains(r.xtbname)).ToList();
            }
            response.Close();
            if (datas.Count > 0)
            {
        ...
        private static void SyncDepartment(string sid, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackDepartment, request);
        }

        private static void AsyncCallbackDepartment(IAsyncResult ar)
        {
            List<LsModel> datas = new List<LsModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<LsModel> data = js.Deserialize<SyncResultModel<LsModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        public static void SyncUser(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackUser, request);
        }

        private static void AsyncCallbackUser(IAsyncResult ar)
        {
            ...
            {
                string result = sr.ReadToEnd();
                SyncResultModel<UserModel> data = js.Deserialize<SyncResultModel<UserModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.AppendLine("select max(xversion) from cnku");
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackCnku, request);
        }

        private static void AsyncCallbackCnku(IAsyncResult ar)
        {
            List<CnkuModel> datas = new List<CnkuModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<CnkuModel> data = js.Deserialize<SyncResultModel<CnkuModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncOfhh(string sid, string xls, string usercode)
        {
            ...
            if (ofhhs.Count > 0)
            {
                ...
                newStream.Close();
                request.BeginGetResponse(AsyncCallbackOfhh_UP, request);
            }
        }
        private static void AsyncCallbackOfhh_UP(IAsyncResult ar)
        {
            List<OfhhModel> datas = new List<OfhhModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<OfhhModel> data = js.Deserialize<SyncResultModel<OfhhModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        #region 当前进销存帐
        private static void Syncku2(string sid, string xls, string usercode)
        {
            StringBuilder cmdText = new StringBuilder();
            ...
            string url = GetUrl(sid, "ku2", xls, usercode, version);

            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackKu2, request);
        }

        private static void AsyncCallbackKu2(IAsyncResult ar)
        {
            List<Ku2Model> datas = new List<Ku2Model>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<Ku2Model> data = js.Deserialize<SyncResultModel<Ku2Model>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        public static void SyncGoodtype1(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackGoodtype1, request);
        }

        private static void AsyncCallbackGoodtype1(IAsyncResult ar)
        {
            List<Goodtype1Model> datas = new List<Goodtype1Model>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<Goodtype1Model> data = js.Deserialize<SyncResultModel<Goodtype1Model>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        public static void SyncGoodtype2(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackGoodtype2, request);
        }

        private static void AsyncCallbackGoodtype2(IAsyncResult ar)
        {
            List<Goodtype2Model> datas = new List<Goodtype2Model>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<Goodtype2Model> data = js.Deserialize<SyncResultModel<Goodtype2Model>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        public static void SyncGoodtype3(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackGoodtype3, request);
        }

        private static void AsyncCallbackGoodtype3(IAsyncResult ar)
        {
            List<Goodtype3Model> datas = new List<Goodtype3Model>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<Goodtype3Model> data = js.Deserialize<SyncResultModel<Goodtype3Model>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        public static void SyncGoodkeys(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackGoodkeys, request);
        }

        private static void AsyncCallbackGoodkeys(IAsyncResult ar)
        {
            List<GoodkeysModel> datas = new List<GoodkeysModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<GoodkeysModel> data = js.Deserialize<SyncResultModel<GoodkeysModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncGood(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackGood, request);
        }

        private static void AsyncCallbackGood(IAsyncResult ar)
        {
            List<GoodModel> datas = new List<GoodModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<GoodModel> data = js.Deserialize<SyncResultModel<GoodModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncGoodpric(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackGoodpric, request);
        }

        private static void AsyncCallbackGoodpric(IAsyncResult ar)
        {
            List<GoodpricModel> datas = new List<GoodpricModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<GoodpricModel> data = js.Deserialize<SyncResultModel<GoodpricModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncGoodpric2(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackGoodpric2, request);
        }

        private static void AsyncCallbackGoodpric2(IAsyncResult ar)
        {
            List<Goodpric2Model> datas = new List<Goodpric2Model>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<Goodpric2Model> data = js.Deserialize<SyncResultModel<Goodpric2Model>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncGbarcode(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackGbarcode, request);
        }

        private static void AsyncCallbackGbarcode(IAsyncResult ar)
        {
            List<GbarcodeModel> datas = new List<GbarcodeModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<GbarcodeModel> data = js.Deserialize<SyncResultModel<GbarcodeModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncGoodkind(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackGoodkind, request);
        }

        private static void AsyncCallbackGoodkind(IAsyncResult ar)
        {
            List<GoodkindModel> datas = new List<GoodkindModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<GoodkindModel> data = js.Deserialize<SyncResultModel<GoodkindModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncClnttype(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackClnttype, request);
        }

        private static void AsyncCallbackClnttype(IAsyncResult ar)
        {
            List<ClnttypeModel> datas = new List<ClnttypeModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<ClnttypeModel> data = js.Deserialize<SyncResultModel<ClnttypeModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncClntclss(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackClntclss, request);
        }

        private static void AsyncCallbackClntclss(IAsyncResult ar)
        {
            List<ClntclssModel> datas = new List<ClntclssModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<ClntclssModel> data = js.Deserialize<SyncResultModel<ClntclssModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncClnt(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackClnt, request);
            wait_Sync.WaitOne();
            ...
            if (clnts.Count > 0)
            {
                ...
                postData += "&data=" + HttpUtility.UrlEncode(json, Encoding.UTF8);
                byte[] byteArray = Encoding.UTF8.GetBytes(postData);
                request = (HttpWebRequest)WebRequest.Create(baseUrl_Push);
                ...
                newStream.Write(byteArray, 0, byteArray.Length);
                newStream.Close();
                request.BeginGetResponse(AsyncCallbackClnt_UP, request);
            }
        }

        private static void AsyncCallbackClnt(IAsyncResult ar)
        {
            List<ClntModel> datas = new List<ClntModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<ClntModel> data = js.Deserialize<SyncResultModel<ClntModel>>(result);
                datas = data.datas;
            }
            response.Close();
        ...
        private static void AsyncCallbackClnt_UP(IAsyncResult ar)
        {
            List<ClntModel> datas = new List<ClntModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
        ...
        private static void SyncJjie2(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackJjie2, request);
        }

        private static void AsyncCallbackJjie2(IAsyncResult ar)
        {
            List<Jjie2Model> datas = new List<Jjie2Model>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<Jjie2Model> data = js.Deserialize<SyncResultModel<Jjie2Model>>(result);
                datas = data.datas;
            }
            response.Close();
        ...
        private static void SyncOjie2(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackOjie2, request);
        }

        private static void AsyncCallbackOjie2(IAsyncResult ar)
        {
            List<Ojie2Model> datas = new List<Ojie2Model>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<Ojie2Model> data = js.Deserialize<SyncResultModel<Ojie2Model>>(result);
                datas = data.datas;
            }
            response.Close();
        ...
        private static void SyncSale(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackSale, request);
        }

        private static void AsyncCallbackSale(IAsyncResult ar)
        {
            List<SaleModel> datas = new List<SaleModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<SaleModel> data = js.Deserialize<SyncResultModel<SaleModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncSalegood(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackSalegood, request);
        }

        private static void AsyncCallbackSalegood(IAsyncResult ar)
        {
            List<SalegoodModel> datas = new List<SalegoodModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<SalegoodModel> data = js.Deserialize<SyncResultModel<SalegoodModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncSalegoodX(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackSalegoodX, request);
        }

        private static void AsyncCallbackSalegoodX(IAsyncResult ar)
        {
            List<SalegoodXModel> datas = new List<SalegoodXModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<SalegoodXModel> data = js.Deserialize<SyncResultModel<SalegoodXModel>>(result);
                datas = data.datas;
            }
            response.Close();
        ...
        private static void SyncSalerule(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackSalerule, request);
        }

        private static void AsyncCallbackSalerule(IAsyncResult ar)
        {
            List<SaleruleModel> datas = new List<SaleruleModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<SaleruleModel> data = js.Deserialize<SyncResultModel<SaleruleModel>>(result);
                datas = data.datas;
            }
            response.Close();
        ...
        private static void SyncPosbantype(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackPosbantype, request);
        }
        private static void AsyncCallbackPosbantype(IAsyncResult ar)
        {
            List<PosbantypeModel> datas = new List<PosbantypeModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<PosbantypeModel> data = js.Deserialize<SyncResultModel<PosbantypeModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncPoshh(string sid, string xls, string usercode)
        {
            ...
            cmdText = new StringBuilder();
            parameters = null;
            cmdText.AppendLine("select * from poshh where SID is null");
            if (version != null && version.ToString() != string.Empty)
            {
                cmdText.AppendLine("or xversion>@xversion");
                parameters = new SQLiteParameter[1];
            ...
            if (poshhs.Count > 0)
            {
                ...
                newStream.Write(byteArray, 0, byteArray.Length);
                newStream.Close();
                request.BeginGetResponse(AsyncCallbackPoshh_UP, request);
            }
        }
        private static void AsyncCallbackPoshh_UP(IAsyncResult ar)
        {
            List<PoshhModel> datas = new List<PoshhModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<PoshhModel> data = js.Deserialize<SyncResultModel<PoshhModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncPosban(string sid, string xls, string usercode)
        {
            ...
            if (posbans.Count > 0)
            {
                ...
                newStream.Write(byteArray, 0, byteArray.Length);
                newStream.Close();
                request.BeginGetResponse(AsyncCallbackPosban_UP, request);
            }
        }
        private static void AsyncCallbackPosban_UP(IAsyncResult ar)
        {
            List<PosbanModel> datas = new List<PosbanModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<PosbanModel> data = js.Deserialize<SyncResultModel<PosbanModel>>(result);
                datas = data.datas;
            }
            request.Abort();
            response.Close();
        ...
        private static void SyncPossetting(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackPossetting, request);
        }

        private static void AsyncCallbackPossetting(IAsyncResult ar)
        {
            List<PossettingModel> datas = new List<PossettingModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<PossettingModel> data = js.Deserialize<SyncResultModel<PossettingModel>>(result);
                datas = data.datas;
            }
            response.Close();
            if (datas.Count > 0)
        ...
        private static void SyncTickoff(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackTickoff, request);
        }

        private static void AsyncCallbackTickoff(IAsyncResult ar)
        {
            List<TickoffModel> datas = new List<TickoffModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<TickoffModel> data = js.Deserialize<SyncResultModel<TickoffModel>>(result);
                datas = data.datas;
            }
            response.Close();
        ...
        private static void SyncTickoffmx(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackTickoffmx, request);
        }

        private static void AsyncCallbackTickoffmx(IAsyncResult ar)
        {
            List<TickoffmxModel> datas = new List<TickoffmxModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<TickoffmxModel> data = js.Deserialize<SyncResultModel<TickoffmxModel>>(result);
                datas = data.datas;
            }
            response.Close();
        ...
        private static void SyncPayt(string sid, string xls, string usercode)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.BeginGetResponse(AsyncCallbackPayt, request);
        }

        private static void AsyncCallbackPayt(IAsyncResult ar)
        {
            List<PaytModel> datas = new List<PaytModel>();
            WebRequest request = ar.AsyncState as WebRequest;
            var response = request.EndGetResponse(ar);
            var stream = response.GetResponseStream();
            using (StreamReader sr = new StreamReader(stream, Encoding.UTF8))
            {
                string result = sr.ReadToEnd();
                SyncResultModel<PaytModel> data = js.Deserialize<SyncResultModel<PaytModel>>(result);
                datas = data.datas;
            }
            response.Close();
