...
{
    public class SyncHelperDAL : BaseDAL
    {
        ...
        static PossettingDAL possettingDAL = new PossettingDAL();
        //static string secretKey = string.Empty;
        static SyncHelperDAL()
        {
            baseUrl = GlobalHelper.GetBaseUrl();
            baseUrl_Pull = GlobalHelper.GetBaseUrl_Pull();
        ...
        internal static string secretKey
        {
            get
            {
                PossettingModel possetting = possettingDAL.GetPossettingByKey(AppConst.SecretKey);
                if (possetting != null)
                {
                    return possetting.xpvalue.Trim();
                };
                return string.Empty;
            }
        }
        ...
        /// <returns></returns>
        private static string GetUrl(string sid, string mod, string xls, string usercode, object version, int id = 0, string goodcode = "", string clntcode = "", int page = 1, int pageSize = 20)
        {
            ...
            string urlFormat = string.Format("{0}?sid={1}&mod={2}&xls={3}&usercode={4}&version={5}&id={6}&goodcode={7}&clntcode={8}&page={9}&pageSize={10}", baseUrl_Pull, sid, mod, xls, usercode, version ?? "", id, goodcode, clntcode, page, pageSize);
            string queryString = new Uri(urlFormat).Query;
            if (!string.IsNullOrEmpty(queryString))
            {
                urlFormat = urlFormat + "&sign=" + GlobalHelper.GetSign(queryString, secretKey);
            }
            return urlFormat;
        }
        #endregion
        ...
        public static bool CheckConnect(out DateTime currentTime, string sid, string username, string password, int second = 5)
        {
            currentTime = DateTime.Parse(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
            try
            {
                string sql = "select CONVERT(varchar(100),GETDATE(),21) as data";
                string url = GlobalHelper.GetBaseUrl_Pull(sid, username, password, sql);
                // Uri baseUrl = new Uri(ConfigurationManager.AppSettings["serviceUrl"].ToString());
                string result = HttpClient.Get(url, 1000 * second).Trim();
                List<SyncResultModel<object>> data = js.Deserialize<List<SyncResultModel<object>>>(result);
                var query = data.Where(r => r.data != null && r.data != "").FirstOrDefault();
                currentTime = Convert.ToDateTime(query.data);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public static bool Cksign(string sid, string xls)
        {
            try
            {
                string urlStr = string.Format(@"pos/cksign?xls={0}&sid={1}&ts={2} ", xls, sid, GetTimeStamp());
                Uri url = new Uri(baseUrl, urlStr);
                string result = HttpClient.Get(url, null, secretKey).Trim();
        ...
        public static decimal Getjifen(string sid, string userCode, string xls)
        {
            ...
            {
                decimal result;
                string urlStr = string.Format(@"pos/jifen?sid={0}&clntcode={1}&xls={2}", sid, userCode, xls);
                Uri url = new Uri(baseUrl, urlStr);
                string r = HttpClient.Get(url, null, secretKey);
                SyncResultModel<object> syncResult = js.Deserialize<SyncResultModel<object>>(r);
                if (syncResult.code == 200013)
                {
                    logger.Info(syncResult.msg);
                }
        ...
        public static decimal Getyck(string sid, string userCode, string xls)
        {
            ...
            {
                decimal result;
                string urlStr = string.Format(@"pos/yck?sid={0}&clntcode={1}&xls={2}", sid, userCode, xls);
                Uri url = new Uri(baseUrl, urlStr);
                string r = HttpClient.Get(url, null, secretKey);
                SyncResultModel<object> syncResult = js.Deserialize<SyncResultModel<object>>(r);
                if (syncResult.code == 200013)
        ...
        public static bool SyncData(string sid, string xls, string usercode, List<string> tables, int id = 0, Guid? posid = null, string goodcode = "", string clntcode = "")
        {
            try
            {
                ...
                {
                    metxt.WaitOne();
                    SyncPoshh(sid, xls, usercode, posid);
                    metxt.ReleaseMutex();
                    //wait_Sync.WaitOne();
                }
                ...
                {
                    metxt.WaitOne();
                    result = Syncku2(sid, xls, usercode, id, goodcode);
                    metxt.ReleaseMutex();
                    //wait_Sync.WaitOne();
                }
                ...
                {
                    metxt.WaitOne();
                    SyncJjie2(sid, xls, usercode, clntcode);
                    metxt.ReleaseMutex();
                    //wait_Sync.WaitOne();
                }
                ...
                {
                    metxt.WaitOne();
                    SyncOjie2(sid, xls, usercode, clntcode);
                    metxt.ReleaseMutex();
                    //wait_Sync.WaitOne();
                }
                ...
                if (tables.Contains("clntday"))
                {
                    metxt.WaitOne();
                    SyncClntDay(sid, xls, usercode);
                    metxt.ReleaseMutex();
                    //wait_Sync.WaitOne();
                }
                if (tables.Contains("clntprice"))
                {
                    metxt.WaitOne();
                    SyncClntPrice(sid, xls, usercode);
                    metxt.ReleaseMutex();
                    //wait_Sync.WaitOne();
                }
        ...
        public static bool SyncData(string sid, string xls, string usercode, string username, string password, List<string> tables, int id = 0, Guid? posid = null, string goodcode = "", string clntcode = "")
        {
            try
            {
                ...
                if (tables.Contains("ls"))
                {
                    ...
                    //{ }
                    metxt.WaitOne();
                    SyncDepartment(sid, username, password);
                    metxt.ReleaseMutex();
                    //wait_Sync.WaitOne();
                }
                if (tables.Contains("user"))
            ...
            if (datas != null && datas.Count > 0)
            {
                ...
                try
                {
                    ...
                    foreach (SyncDelDetailModel item in datas)
                    {
                        ...
                        SqlExpressionModel sqlExp = new SqlExpressionModel();
                        sqlExp.CmdText = cmdText.ToString();
                        sqlExp.Parameters = parameters;
                    ...
                    long result = (long)SQLiteHelper.ExecuteScalar(SQLiteHelper.connectionString, CommandType.Text, cmdText, parameters);
                    ...
                    if (result == 0)
                    {
                        ...
                        SqlExpressionModel sqlExp = new SqlExpressionModel();
                        sqlExp.CmdText = cmdText_Add.ToString();
                        sqlExp.Parameters = parameters_Add;
                        sqlStringList.Add(sqlExp);
                    }
                    else
                    {
        ...
        #region 同步分部
        private static void SyncDepartment(string sid, string username, string password)
        {
            wait_Sync = new ManualResetEvent(false);
            StringBuilder cmdText = new StringBuilder();
            ...
            string url = GlobalHelper.GetBaseUrl_Pull(sid, username, password, GetSHOWSQL("ls", version, out filename));
            //GetSHOWSQL
            //   SyncResultModel<LsModel> syncResult = HttpClient.GetAasync<LsModel>(url, null);
            try
            {
                HttpClient.GetAasync(url, null, syncResult =>
                  {
                      List<LsModel> result = GetConverToData<LsModel>(filename, syncResult);
                      //LsModel
                      AsyncCallbackDepartment(result);
                      wait_Sync.Set();
                  });
                wait_Sync.WaitOne();
            }
        ...
        private static void AsyncCallbackDepartment(List<LsModel> datas)
        {
            if (datas != null && datas.Count > 0)
            {
                #region 同步到数据库
        ...
        private static void SyncOfhh(string sid, string xls, string usercode)
        {
            //上传
            StringBuilder cmdText = new StringBuilder();
            cmdText.AppendLine("select * from ofhh where SID is null and ifnull(uploadstatus,'0')='0'");
            ...
            dataReader.Close();
            List<Guid> ids = ofhhs.Select(r => r.ID).ToList();
            ReturnUploadstatus("ofhh", ((int)UploadStatus.Uploading).ToString(), ids);
            ...
            if (ofhhs.Count > 0)
            {
                ...
                string postData = GlobalHelper.GetPostData(sid, "ofhh", xls, usercode, json, secretKey);
                try
                {
                    SyncResultModel<OfhhModel> syncResult = HttpClient.PostAasync<OfhhModel>(baseUrl_Push.ToString(), null, postData);
                    AsyncCallbackOfhh_UP(syncResult, ids);
                }
                catch (Exception ex)
                {
                    ReturnUploadstatus("ofhh", ((int)UploadStatus.NotUploaded).ToString(), ids);
                    throw ex;
                }
            }
        }
        private static void AsyncCallbackOfhh_UP(SyncResultModel<OfhhModel> syncResult, List<Guid> ids)
        {
            ...
            if (datas != null && datas.Count > 0)
            {
                ...
                try
                {
                    ...
                    SQLiteHelper.ExecuteSqlTran(sqlStringList);
                    ReturnUploadstatus("ofhh", ((int)UploadStatus.Uploaded).ToString(), ids);
                }
                catch (Exception ex)
                {
                    ReturnUploadstatus("ofhh", ((int)UploadStatus.NotUploaded).ToString(), ids);
                ...
                {
                    //wait_Sync.Set();
                    //string cmdText = string.Format("update ofhh set uploadstatus ='{0}' where ID in ('{1}')", string.Empty, string.Join("','", datas.Select(r => r.ID)));
                    //SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText);
                    //ReturnUploadstatus("ofhh", ((int)UploadStatus.Uploaded).ToString(), datas.Select(r => r.ID).ToList());
                }
                #endregion
            }
            //wait_Sync.Set();
        ...
        #region 当前进销存帐
        private static bool Syncku2(string sid, string xls, string usercode, int id = 0, string goodcode = "")
        {
            wait_Sync = new ManualResetEvent(false);
            StringBuilder cmdText = new StringBuilder();
            ...
            object version = SQLiteHelper.ExecuteScalar(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString());
            string url = GetUrl(sid, "ku2", xls, usercode, version, id, goodcode);
            ...
            try
            {
               ...
               {
                   if (syncResult.msg == "超时")
                   {
                       result = false;
                   }
                   AsyncCallbackKu2(syncResult, id, goodcode);
                   wait_Sync.Set();
               });
                wait_Sync.WaitOne();
                return result;
        ...
        private static void AsyncCallbackKu2(SyncResultModel<Ku2Model> syncResult, int id, string goodcode)
        {
                        ...
                        else
                        {
                            ...
                            str.AppendLine(@"UPDATE ku2
                                               SET xls = @xls,
                                                   ...
                                                   xtableid = @xtableid,
                                                   xsubid = @xsubid ");
                            if (id == 0 || string.IsNullOrEmpty(goodcode))
                            {
                                str.AppendLine(",xversion = @xversion ");
                            }
                            str.AppendLine("WHERE SID = @SID");
        ...
        private static void SyncClnt(string sid, string xls, string usercode)
        {
            ...
            //上传
            cmdText = new StringBuilder();
            cmdText.AppendLine("select * from clnt where (SID is null and (ifnull(uploadstatus,'0')='0' or uploadstatus='')) or isUpdate =1");
            ...
            dataReader.Close();
            List<Guid> ids = clnts.Select(r => r.ID).ToList();
            //cmdText = new StringBuilder();
            //cmdText.AppendFormat("update clnt set uploadstatus ='{0}' where ID in ('{1}')", (int)UploadStatus.Uploading, string.Join("','", ids));
            //SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString());
            ReturnUploadstatus("clnt", ((int)UploadStatus.Uploading).ToString(), ids);
            if (clnts.Count > 0)
            {
                ...
                try
                {
                    ...
                    AsyncCallbackClnt_UP(syncResult, ids);
                }
                catch (Exception ex)
                {
                    //cmdText = new StringBuilder();
                    //cmdText.AppendFormat("update clnt set uploadstatus ='{0}' where ID in ('{1}')", string.Empty, string.Join("','", ids));
                    //SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString());
                    ReturnUploadstatus("clnt", ((int)UploadStatus.NotUploaded).ToString(), ids);
                    throw ex;
                }
            }
        }
        ...
        private static void AsyncCallbackClnt_UP(SyncResultModel<ClntModel> syncResult, List<Guid> ids)
        {
            List<ClntModel> datas = syncResult.datas;
            if (datas != null && datas.Count > 0)
            {
                ...
                try
                {
                    ...
                    SQLiteHelper.ExecuteSqlTran(sqlStringList);
                    ReturnUploadstatus("clnt", ((int)UploadStatus.Uploaded).ToString(), ids);
                }
                catch (Exception ex)
                {
                    ReturnUploadstatus("clnt", ((int)UploadStatus.NotUploaded).ToString(), ids);
                ...
                {
                    //string cmdText = string.Format("update clnt set uploadstatus ='{0}' where ID in ('{1}')", string.Empty, string.Join("','", datas.Select(r => r.ID)));
                    //SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText);
                    //ReturnUploadstatus("clnt", ((int)UploadStatus.Uploaded).ToString(), datas.Select(r => r.ID).ToList());
                    //wait_Sync.Set();
        ...
        #region 积分结余
        private static void SyncJjie2(string sid, string xls, string usercode, string clntcode)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.AppendFormat("select max(xversion) from jjie2");
            object version = SQLiteHelper.ExecuteScalar(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString());
            string url = GetUrl(sid, "jjie2", xls, usercode, version, 0, string.Empty, clntcode);
        ...
        #region 预存款结余
        private static void SyncOjie2(string sid, string xls, string usercode, string clntcode)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.AppendFormat("select max(xversion) from ojie2");
            object version = SQLiteHelper.ExecuteScalar(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString());
            string url = GetUrl(sid, "ojie2", xls, usercode, version, 0, string.Empty, clntcode);
        ...
        private static void AsyncCallbackSale(SyncResultModel<SaleModel> syncResult)
        {
            ...
            if (datas != null && datas.Count > 0)
            {
                ...
                try
                {
                    ...
                    sqlExp = new SqlExpressionModel();
                    sqlExp.CmdText = cmdText1.ToString();
                    sqlExp.Parameters = parameters1;
                    sqlStringList.Add(sqlExp);
                    SQLiteHelper.ExecuteSqlTran(sqlStringList);
                }
                catch (Exception ex)
        ...
        private static void SyncPoshh(string sid, string xls, string usercode, Guid? id = null)
        {
            ...
            if (id.HasValue)
            {
                ...
                cmdText.AppendLine("select * from poshh where ID =@ID and xstate !=@xstate ");
            }
            else
            {
                cmdText.AppendLine("select * from poshh where SID is null and xstate !=@xstate and (ifnull(uploadstatus,'0')='0' or uploadstatus='')");
                //if (version != null && version.ToString() != string.Empty)
                //{
                cmdText.AppendLine("or (SID is not null and xstate !=@xstate and uploadstatus=@uploadstatus)");
                parameters = new SQLiteParameter[2];
                ...
                parameters[1] = new SQLiteParameter("uploadstatus", DbType.String);
                parameters[1].Value = (int)UploadStatus.NotUploaded;
                // }
                cmdText.AppendLine(" limit 5");
            }
            ...
            while (dataReader.Read())
            {
                ...
                poshh.xheallp = poshh.xstate == "退货" ? 0 - decimal.Parse(dataReader["xheallp"].ToString().Trim()) : decimal.Parse(dataReader["xheallp"].ToString().Trim());
                poshh.xpay = poshh.xstate == "退货" ? 0 - decimal.Parse(dataReader["xpay"].ToString().Trim()) : decimal.Parse(dataReader["xpay"].ToString().Trim());
                poshh.xhezhe = decimal.Parse(dataReader["xhezhe"].ToString().Trim());
                poshh.xhenojie = poshh.xstate == "退货" ? 0 - decimal.Parse(dataReader["xhenojie"].ToString().Trim()) : decimal.Parse(dataReader["xhenojie"].ToString().Trim());
                ...
                poshh.xintime = Convert.ToDateTime(dataReader["xintime"].ToString()).ToString("yyyy-MM-dd HH:mm:ss");
                ...
                poshh.xrpay = (dataReader["xrpay"] == null || string.IsNullOrEmpty(dataReader["xrpay"].ToString())) ? 0 : (poshh.xstate == "退货" ? 0 - decimal.Parse(dataReader["xrpay"].ToString()) : decimal.Parse(dataReader["xrpay"].ToString()));
            ...
            dataReader.Close();
            cmdText = new StringBuilder();
            List<Guid> ids = poshhs.Select(r => r.ID).ToList();
            //cmdText.AppendFormat("update poshh set uploadstatus ='{0}' where ID in ('{1}')", (int)UploadStatus.Uploading, string.Join("','", ids));
            //SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString());
            ReturnUploadstatus("poshh", ((int)UploadStatus.Uploading).ToString(), ids);
            ...
            foreach (var item in poshhs)
            {
                ...
                while (dataReader_detail.Read())
                {
                    ...
                    posbb.unitquat = (dataReader_detail["unitquat"] != null && dataReader_detail["unitquat"].ToString().Trim() != string.Empty)
                        ? (item.xstate == "退货" ? 0 - decimal.Parse(dataReader_detail["unitquat"].ToString().Trim()) : decimal.Parse(dataReader_detail["unitquat"].ToString().Trim()))
                        : 0;
                    ...
                    posbb.xquat = item.xstate == "退货" ? 0 - decimal.Parse(dataReader_detail["xquat"].ToString().Trim()) : decimal.Parse(dataReader_detail["xquat"].ToString().Trim());
                    ...
                    posbb.xallp = item.xstate == "退货" ? 0 - decimal.Parse(dataReader_detail["xallp"].ToString().Trim()) : decimal.Parse(dataReader_detail["xallp"].ToString().Trim());
                    posbb.xtaxr = decimal.Parse(dataReader_detail["xtaxr"].ToString().Trim());
                    posbb.xtax = item.xstate == "退货" ? 0 - decimal.Parse(dataReader_detail["xtax"].ToString().Trim()) : decimal.Parse(dataReader_detail["xtax"].ToString().Trim());
                    //posbb.xprict = decimal.Parse(dataReader_detail["xprict"].ToString().Trim());
                    posbb.xallpt = item.xstate == "退货" ? 0 - decimal.Parse(dataReader_detail["xallpt"].ToString().Trim()) : decimal.Parse(dataReader_detail["xallpt"].ToString().Trim());
                ...
                while (dataReader_pay.Read())
                {
                    ...
                    payt.xpay = item.xstate == "退货" ? 0 - decimal.Parse(dataReader_pay["xpay"].ToString().Trim()) : decimal.Parse(dataReader_pay["xpay"].ToString().Trim());
            ...
            if (poshhs.Count > 0)
            {
                ...
                //postData += "&usercode=" + usercode;
                //postData += "&data=" + HttpUtility.UrlEncode(json, Encoding.UTF8);
                string postData = GlobalHelper.GetPostData(sid, "poshh", xls, usercode, json, secretKey);

                try
                {
                    SyncResultModel<PoshhModel> syncResult = HttpClient.PostAasync<PoshhModel>(baseUrl_Push.ToString(), null, postData);
                    AsyncCallbackPoshh_UP(syncResult, ids);
                }
                catch (Exception ex)
                {
                    ReturnUploadstatus("poshh", ((int)UploadStatus.NotUploaded).ToString(), ids);
                    throw ex;
                }
            }
        }
        private static void AsyncCallbackPoshh_UP(SyncResultModel<PoshhModel> syncResult, List<Guid> ids)
        {
            ...
            if (datas != null && datas.Count > 0)
            {
                ...
                try
                {
                    ...
                    SQLiteHelper.ExecuteSqlTran(sqlStringList);
                    ReturnUploadstatus("poshh", ((int)UploadStatus.Uploaded).ToString(), ids);
                }
                catch (Exception ex)
                {
                    ReturnUploadstatus("poshh", ((int)UploadStatus.NotUploaded).ToString(), ids);
                ...
                {
                    //string cmdText = string.Format("update poshh set uploadstatus ='{0}' where ID in ('{1}')", string.Empty, string.Join("','", datas.Select(r => r.ID)));
                    //SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText);
                    // ReturnUploadstatus("poshh", ((int)UploadStatus.Uploaded).ToString(), datas.Select(r => r.ID).ToList());
                }
                #endregion
            }
            else if (syncResult.msg == "成功")
            {
                ReturnUploadstatus("poshh", ((int)UploadStatus.Uploaded).ToString(), ids);
            }
            else
        ...
        private static void SyncPosban(string sid, string xls, string usercode)
        {
            //上传
            SQLiteParameter[] parameters = new SQLiteParameter[1];
            parameters[0] = new SQLiteParameter("uploadstatus", DbType.String);
            parameters[0].Value = (int)UploadStatus.NotUploaded;
            StringBuilder cmdText = new StringBuilder();
            cmdText.AppendLine("select * from posban where SID is null and xjieok=1 and ifnull(uploadstatus,'0')='0' ");
            cmdText.AppendLine("or (SID is not null and xjieok=1 and uploadstatus=@uploadstatus) ");
            SQLiteDataReader dataReader = SQLiteHelper.ExecuteReader(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString(), parameters);
            ...
            dataReader.Close();
            cmdText = new StringBuilder();
            List<Guid> ids = posbans.Select(r => r.ID).ToList();
            //cmdText.AppendFormat("update posban set uploadstatus ='{0}' where ID in ('{1}')", (int)UploadStatus.Uploading, string.Join("','", ids));
            //SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString());
            ReturnUploadstatus("posban", ((int)UploadStatus.Uploading).ToString(), ids);
            if (posbans.Count > 0)
            {
                ...
                //postData += "&usercode=" + usercode;
                //postData += "&data=" + HttpUtility.UrlEncode(json, Encoding.UTF8);
                string postData = GlobalHelper.GetPostData(sid, "posban", xls, usercode, json, secretKey);
                byte[] byteArray = Encoding.UTF8.GetBytes(postData);

                try
                {
                    SyncResultModel<PosbanModel> syncResult = HttpClient.PostAasync<PosbanModel>(baseUrl_Push.ToString(), null, postData);
                    AsyncCallbackPosban_UP(syncResult, ids);
                }
                catch (Exception ex)
                {
                    //cmdText = new StringBuilder();
                    //cmdText.AppendFormat("update posban set uploadstatus ='{0}' where ID in ('{1}')", string.Empty, string.Join("','", ids));
                    //SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString());
                    ReturnUploadstatus("posban", ((int)UploadStatus.NotUploaded).ToString(), ids);
                    throw ex;
                }
            }
        }
        private static void AsyncCallbackPosban_UP(SyncResultModel<PosbanModel> syncResult, List<Guid> ids)
        {
            ...
            if (datas != null && datas.Count > 0)
            {
                ...
                try
                {
                    ...
                    SQLiteHelper.ExecuteSqlTran(sqlStringList);
                    ReturnUploadstatus("posban", ((int)UploadStatus.Uploaded).ToString(), ids);
                }
                catch (Exception ex)
                {
                    ReturnUploadstatus("posban", ((int)UploadStatus.NotUploaded).ToString(), ids);
                ...
                {
                    //ReturnUploadstatus("posban", ((int)UploadStatus.Uploaded).ToString(), datas.Select(r => r.ID).ToList());
                    //string cmdText = string.Format("update posban set uploadstatus ='{0}' where ID in ('{1}')", string.Empty, string.Join("','", datas.Select(r => r.ID)));
                    //SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText);
                }
                #endregion
            }
        ...
        #region  会员日
        private static void SyncClntDay(string sid, string xls, string usercode)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.AppendFormat("select max(xversion) from clntday");
            object version = SQLiteHelper.ExecuteScalar(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString());
            string url = GetUrl(sid, "clntday", xls, usercode, version);
            //SyncResultModel<PaytModel> syncResult = HttpClient.GetAasync<PaytModel>(url, null);
            //AsyncCallbackPayt(syncResult);
            try
            {
                HttpClient.GetAasync<ClntDayModel>(url, null, syncResult =>
               {
                   AsyncCallbackClntDay(syncResult);
               });
            }
            catch (Exception ex)
            {
        ...
        private static void AsyncCallbackClntDay(SyncResultModel<ClntDayModel> syncResult)
        {
            List<ClntDayModel> datas = syncResult.datas;
            if (datas != null && datas.Count > 0)
            {
                List<SqlExpressionModel> sqlStringList = new List<SqlExpressionModel>();
                ...
                try
                {
                    foreach (ClntDayModel item in datas)
                    {
                        string cmdText = "select count(*) from clntday where SID=@SID";
                        SQLiteParameter[] parameters = new SQLiteParameter[1];
                        parameters[0] = new SQLiteParameter("SID", DbType.Int32);
                        parameters[0].Value = item.SID;
                        long result = (long)SQLiteHelper.ExecuteScalar(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString(), parameters);
                        ...
                        parameters[0] = new SQLiteParameter("SID", DbType.Int32);
                        parameters[0].Value = item.SID;
                        parameters[1] = new SQLiteParameter("xdate", DbType.Int16);
                        parameters[1].Value = item.xday;
                        parameters[2] = new SQLiteParameter("xstart", DbType.Boolean);
                        parameters[2].Value = item.xstart;
                        parameters[3] = new SQLiteParameter("xtableid", DbType.Int32);
                        parameters[3].Value = item.xtableid;
                        parameters[4] = new SQLiteParameter("xsubid", DbType.Int32);
                        parameters[4].Value = item.xsubid;
                        parameters[5] = new SQLiteParameter("xversion", DbType.Double);
                        parameters[5].Value = item.xversion;
                        parameters[6] = new SQLiteParameter("xmonth", DbType.String);
                        parameters[6].Value = item.xmonth;


                        if (result == 0)
                        {
                            cmdText = @"INSERT INTO clntday (
                                        SID,
                                        xday,
                                        xstart,
                                        xtableid,
        ...
        #region  会员日分类等级折扣
        private static void SyncClntPrice(string sid, string xls, string usercode)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.AppendFormat("select max(xversion) from clntdrule");
            object version = SQLiteHelper.ExecuteScalar(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString());
            string url = GetUrl(sid, "clntdrule", xls, usercode, version);
            //SyncResultModel<PaytModel> syncResult = HttpClient.GetAasync<PaytModel>(url, null);
            //AsyncCallbackPayt(syncResult);
            try
            {
                HttpClient.GetAasync<ClntdRuleModel>(url, null, syncResult =>
               {
                   AsyncCallbackClntPrice(syncResult);
               });
            }
            catch (Exception ex)
            {
        ...
        private static void AsyncCallbackClntPrice(SyncResultModel<ClntdRuleModel> syncResult)
        {
            List<ClntdRuleModel> datas = syncResult.datas;
            if (datas != null && datas.Count > 0)
            {
                List<SqlExpressionModel> sqlStringList = new List<SqlExpressionModel>();
                ...
                try
                {
                    foreach (ClntdRuleModel item in datas)
                    {
                        string cmdText = "select count(*) from clntdrule where SID=@SID";
                        SQLiteParameter[] parameters = new SQLiteParameter[1];
                        parameters[0] = new SQLiteParameter("SID", DbType.Int32);
                        parameters[0].Value = item.SID;
                        long result = (long)SQLiteHelper.ExecuteScalar(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString(), parameters);
                        ...
                        parameters[0] = new SQLiteParameter("SID", DbType.Int32);
                        parameters[0].Value = item.SID;
                        parameters[1] = new SQLiteParameter("goodtype", DbType.String);
                        parameters[1].Value = item.goodtype;
                        parameters[2] = new SQLiteParameter("uclssprics", DbType.String);
                        parameters[2].Value = item.uclssprics;
                        parameters[3] = new SQLiteParameter("classtype", DbType.String);
                        parameters[3].Value = item.classtype;
                        parameters[4] = new SQLiteParameter("xtimes", DbType.Decimal);
                        parameters[4].Value = item.xtimes;
                        parameters[5] = new SQLiteParameter("xtableid", DbType.Int32);
                        parameters[5].Value = item.xtableid;
                        parameters[6] = new SQLiteParameter("xsubid", DbType.Int32);
                        parameters[6].Value = item.xsubid;
        ...
        public static void UploadErrLog(string msg, string sid, string xls, string usercode)
        {
            try
            {
                ...
                //    string postData = GlobalHelper.GetPostData(dic, secretKey);
        ...
        #region 修改单据上传状态
        private static void ReturnUploadstatus(string tablename, string uploadstatus, List<Guid> ids)
        {
            string cmdText = string.Format("update {0} set uploadstatus ='{1}' where ID in ('{2}')", tablename, uploadstatus, string.Join("','", ids));
            SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText);
        }
        #endregion
        ...
        {
            string cmdText = "update poshh set uploadstatus ='0' where ifnull(uploadstatus,'') = '1'";
            SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText);
            cmdText = "update posban set uploadstatus ='0' where ifnull(uploadstatus,'') = '1'";
            SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText);
            cmdText = "update ofhh set uploadstatus ='0' where ifnull(uploadstatus,'') = '1'";
            SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText);
            cmdText = "update clnt set uploadstatus ='0' where ifnull(uploadstatus,'') = '1'";
            SQLiteHelper.ExecuteNonQuery(SQLiteHelper.connectionString, CommandType.Text, cmdText);
        }
        #endregion

        private static string GetSHOWSQL(string tableName, object version, out string filename)
        {
            decimal xversion = version == null || version.ToString().Trim() == string.Empty ? 0 : decimal.Parse(version.ToString());
            filename = string.Empty;
            DirectoryInfo d = new DirectoryInfo(AppConst.dbMapPath);
            FileInfo[] files = d.GetFiles();
            StringBuilder sql = new StringBuilder();
            var query = files.Where(r => r.Name.Split('&')[0] == tableName).FirstOrDefault();
            if (query != null)
            {
                filename = query.FullName;
                int pageSize = 20;
                string pageSet = ConfigurationManager.AppSettings["pageSize"].ToString();
                int size = 0;
                if (int.TryParse(pageSet, out size))
                ...
                List<DBRelation> data = ConfigHelper.LoadConfig<DBRelation>(query.FullName);
                sql.AppendFormat("select top({0}) ", pageSize);
                foreach (DBRelation item in data)
                {
                    sql.AppendFormat("{0},", item.SColumn.Replace("version", "CONVERT(bigint,version) as version"));
                }
                string str = sql.ToString().TrimEnd(',');
                sql = new StringBuilder();
                sql.AppendLine(str);
                sql.AppendFormat(" from {0}", query.Name.Replace(query.Extension, "").Split('&')[1]);
                sql.AppendFormat(" where CONVERT(bigint,version)>{0}", xversion);
            }
            return sql.ToString();
        }

        private static List<T> GetConverToData<T>(string fileName, string json)
        {
            if (json == "超时" && json.Trim() == string.Empty)
            {
                return null;
            }
            List<DBRelation> dbRelations = ConfigHelper.LoadConfig<DBRelation>(fileName);
            foreach (DBRelation item in dbRelations)
            {
                json = json.Replace(item.SColumn, item.LColumn);
            }
            List<T> data = js.Deserialize<List<T>>(json);
            return data;
        }
    }
