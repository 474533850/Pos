...
namespace POS.DAL.Sync
{
    public class SyncHelperDAL : BaseDAL
    {
        ...
        /// <returns></returns>
        private static string GetUrl(string sid, string mod, string xls, string usercode, object version, int id = 0)
        {
            string urlFormat = string.Format("{0}?sid={1}&mod={2}&xls={3}&usercode={4}&version={5}&id={6}", baseUrl_Pull, sid, mod, xls, usercode, version ?? "", id);
            return urlFormat;
        }
        #endregion
        ...
        #region 同步数据
        private static void SyncData(string sid, string usercode,string table)
        {
            string modelName =string.Format("{0}Model", table.Substring(0, 1).ToUpper() + table.Substring(1));
            string className = string.Format("POS.Model.{0}", modelName);
            Assembly asmb = Assembly.LoadFrom("POS.Model.dll");
            Type supType = asmb.GetType(className);
         //   Type type = Type.GetType();
            StringBuilder cmdText = new StringBuilder();
            cmdText.AppendFormat("select max(xversion) from {0}",table);
            object version = SQLiteHelper.ExecuteScalar(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString());
            string url = GetUrl(sid, table, "", usercode, version);
            //   SyncResultModel<LsModel> syncResult = HttpClient.GetAasync<LsModel>(url, null);
            try
            {
                Type httpClient = typeof(HttpClient);

                MethodInfo asyncMethod = httpClient.GetMethod("GetAasync");
               ...
               object tempList = (object)asyncMethod.Invoke(httpClient, new object[] { url, null });
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        //private static void AsyncCallback<T>(SyncResultModel<T> syncResult)
        //{
        ...
        private static void Syncku2(string sid, string xls, string usercode, int id = 0)
        {
            ...
            cmdText.AppendFormat("select xversion from(select xversion from ku2 order by xversion desc limit 0, 10) order by xversion asc limit 0, 1");
            object version = SQLiteHelper.ExecuteScalar(SQLiteHelper.connectionString, CommandType.Text, cmdText.ToString());
